...
class SPStatusError(OAuthError):
    """exception for HTTP Status"""

    def __init__(self, message='Service Provider error.', httpstatus=None):
        super(SPStatusError, self).__init__(message=message)
    ...
    def __str__(self):
        ...
        if self.httpstatus is not None:
            h = " code: %i" % self.httpstatus
        return "%s%s" % (self._message, h)


class HTTPError(OAuthError):
    """exception for URL fetch"""

    def __init__(self,  original_error, message='HTTP Communication error.'):
        super(SPStatusError, self).__init__(message=message)
    ...
    def __str__(self):
        return "%s Error type:%s" % (self._message, self.original_error.__class__.__name__)


class MissingSignature(OAuthError):
    pass
...
class Request(dict):
    ...
    def get_parameter(self, parameter):
        ...
        if ret is None:
            raise KeyError('Parameter not found: %s' % parameter)
        ...
        if headers and 'Authorization' in headers:
            ...
            if auth_header[:6] == 'OAuth ':
                ...
                try:
                    ...
                    raise ValueError('Unable to parse OAuth parameters from '
...
class Server(object):
    ...
    def _check_signature(self, request, consumer, token):
        ...
        if not valid:
            ...
            raise ValueError('Invalid signature. Expected signature base ' 
                             'string: %s' % base)
...
def _urlfetch_with_retry(max_retries=3, retry_status=[], *args, **kwargs):
    times = 0
    r, ex = None, None
    while times <= max_retries:
        ...
        r, ex = None, None
        try:
            resp = urlfetch.fetch(*args, **kwargs)
...
class Client(object):
    ...
    def request(self, uri, method="GET", body=None, headers=None, 
                force_auth_header=False, follow_redirects=True):
        ...
        return _urlfetch_with_retry(retry_status=[401, 403, 500],
                                    url=uri, payload=body, method=method, headers=headers,
                                    follow_redirects=follow_redirects, deadline=self.deadline)
